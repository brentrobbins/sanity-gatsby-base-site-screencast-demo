...
export const query = graphql`
  query($slug: String!, $today: Date!) {
    ...
    filter: { slug: { current: { ne: null } }, title: { ne: null }, publishedAt: {local: {lte: $today}}, category: {slug: {current: {eq: $slug}}} }
    ) {
      edges {
        node {
          id
...
`
const CategoryPage = ({data: {sanityCategory: page, allSanityPost: posts}}) => {
